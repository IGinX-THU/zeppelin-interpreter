...
public class LLMUtils {
   ...
   * @return
   */
  private static String getStandardJson(String json) {

    int start = json.indexOf('{');
    int end = json.lastIndexOf('}');
    String res = null;
    if (start != -1 && end != -1 && start < end) {
      res = json.substring(start, end + 1);
    }
    //        res = res.replace("json", "").replaceAll("```", "");
    return res;
  }
  ...
  public static String getConcept(List<TreeNode> nodes) {
    ...
    //        String res = getResponse("你是一个概括大师，我将给你多个短语，中间用';'来分隔，请给概括这些短语，返回一个短语。" +
    //                "注意只需返回这个短语即可，短语使用大括号进行包裹。需要你合并概括的多和短语是：" + conceptName );
    String res =
        getResponse(
  ...
  private static String getStandardConcept(String text) {
